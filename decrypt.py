...
logging.basicConfig(filename='dec.log', filemode='a', format='%(asctime)s [%(levelname)s] %(message)s ', datefmt='%d-%b-%y %H:%M:%S', level=logging.DEBUG)
logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
logging.info('dec started')
...
if not os.path.exists("dec.lock"):
    with open('dec.lock', 'w') as fp:
        pass
else:
    logging.error("dec.lock present, exitnig...")
    sys.exit(-1)
...
def dec (destination,decrypt):
    files = glob.glob(destination + '/**/*.*', recursive=True)
    for source_file in files:
        destination_file=source_file
        test_file=source_file.replace(destination,decrypt)
        test_file=test_file.replace('.gpg','')
        print (source_file)
...
#                 cmd='gpg -vv --batch --passphrase '+SECRET +' --output \''+destination_file+'.tmp\''+' --symmetric \''+source_file+'\''
            cmd_test='gpg -vv --batch --passphrase '+SECRET +' --output \''+test_file+'\''+' --decrypt \''+destination_file+'\''
#            print(cmd)
            print(cmd_test)
#            sys.exit()
...
with open("decrypt.yml", "r") as stream:
    try:
       ...
       for i in range(0,l):
           ...
           decrypt=y[i]["service"]["decrypt"]
           dec(destination,decrypt)
